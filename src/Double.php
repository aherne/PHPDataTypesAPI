...
final class Double extends Object implements Comparable
{
    ...
    {
        if (!is_float($dblInput)) {
            throw new DataTypeException("Variable is not double: ".$dblInput);
        }
    ...
    {
        if (!($objDouble instanceof Double)) {
            throw new DataTypeException("Compared argument must be of Double type!");
        }
        $dblA = $this->_value_;
