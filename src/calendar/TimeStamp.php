...
final class TimeStamp extends Object implements Comparable {
	...
	public function __construct($strDateTime) {
		if($strDateTime!=null && !self::isValid($strDateTime)) throw new DataTypeException("Invalid timestamp: ".$strTime."! Accepted format is: YYYY-MM-DD HH:MM:SS");
	...
	public function diff($objTimeStamp) {
		if(!($objTimeStamp instanceof TimeStamp)) throw new DataTypeException("Argument must be of TimeStamp type!");
	...
	public function compareTo($objTimeStamp) {
		if(!($objTimeStamp instanceof TimeStamp)) throw new DataTypeException("Compared argument must be of TimeStamp type!");
	...
	public static function isValid($strDateTime) {
		if(preg_match("/(\d{4})-(\d{2})-(\d{2})\ (\d{2}):(\d{2}):(\d{2})/", $strDateTime)==1 && strtotime($strDateTime)!=false ) return true;
		else return false;
	}
