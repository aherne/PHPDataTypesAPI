...
final class Time extends Object
{
    ...
    {
        if ($strTime!=null && !self::isValid($strTime)) {
            throw new DataTypeException("Invalid time: ".$strTime."! Accepted format is: HH:MM:SS");
        }
    ...
    {
        if (preg_match("/(\d{2}):(\d{2}):(\d{2})/", $strTime)==1 && strtotime($strTime)!=false) {
            return true;
        } else {
            return false;
        }
