...
final class Date extends Object implements Comparable
{
    ...
    {
        if ($strDate!=null && !self::isValid($strDate)) {
            throw new DataTypeException("Invalid date: ".$strDate."! Accepted format is: YYYY-MM-DD");
        }
    ...
    public function diff($objDate)
    {
        if (!($objDate instanceof Date)) {
            throw new DataTypeException("Argument must be of Date type!");
        }
    ...
    {
        if (!($objDate instanceof Date)) {
            throw new DataTypeException("Compared argument must be of Date type!");
        }
    ...
    {
        if (preg_match("/(\d{4})-(\d{2})-(\d{2})/", $strDate)==1 && strtotime($strDate)!=false) {
            return true;
        } else {
            return false;
        }
