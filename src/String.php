...
final class String extends Object implements Comparable
{
    ...
    public function __construct($strInput="")
    {
        if (!is_string($strInput)) {
            throw new DataTypeException("Variable is not string: ".$strInput);
        }
    ...
    public function compareTo($objString)
    {
        if (!($objString instanceof String)) {
            throw new DataTypeException("Compared argument must be of String type!");
        }
    ...
    {
        if (!$strCharset) {
            return strlen($this->_value_);
        } else {
            return mb_strlen($this->_value_, $strCharset);
        }
    }
    ...
    public function substring($intPositionStart, $intPositionEnd=0, $strCharset="")
    {
        ...
        if ($intPositionEnd==0) {
            if (!$strCharset) {
                $strOutput = substr($this->_value_, $intPositionStart);
            } else {
                $strOutput = mb_substr($this->_value_, $intPositionStart, $strCharset);
            }
        } elseif ($intPositionStart>=0 && $intPositionEnd > 0) {
            if ($intPositionEnd<$intPositionStart) {
                throw new DatatypeException("Index out of bounds!");
            }
            if (!$strCharset) {
                $strOutput = substr($this->_value_, $intPositionStart, ($intPositionEnd-$intPositionStart));
            } else {
                $strOutput = mb_substr($this->_value_, $intPositionStart, ($intPositionEnd-$intPositionStart), $strCharset);
            }
        } else {
