...
final class Boolean extends Object implements Comparable {
	...
	public function __construct($blnInput=false) {
		if(!is_bool($blnInput)) throw new DataTypeException("Variable is not boolean: ".$blnInput);
	...
	public function compareTo($objBoolean) {
		if(!($objBoolean instanceof Boolean)) throw new DataTypeException("Compared argument must be of Boolean type!");
		$blnA = $this->_value_;
		$blnB = $objBoolean->toValue();
		if($blnA==$blnB) 			return 0;
		else if($blnA<$blnB)		return -1;
		else						return 1;
	}
}